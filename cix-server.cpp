...
void reply_get(accepted_socket& client_sock, cix_header& header){
   //log<<"enter reply_get" << endl;
   ifstream is(header.cix_filename, ifstream::binary);
   log << "filename is: "<<header.cix_filename << endl;
   if(is){
      is.seekg(0, is.end);
      int length = is.tellg();
      is.seekg(0, is.beg);
     ...
     is.read(buffer, length);
      ...
      if(buffer[0] != '\0'){
         ...
         header.cix_nbytes = sizeof buffer;
         send_packet(client_sock, &header, sizeof header);
...
void reply_put(accepted_socket& client_sock, cix_header& header){
   ...
   if(header.cix_filename[0] == ' '){
      ...
      if(header.cix_nbytes != 0){
         ...
         output.write(buffer, header.cix_nbytes);

         header.cix_command = CIX_ACK;
         header.cix_nbytes = sizeof header;
         send_packet(client_sock, &header, sizeof header);
      }else{
         ofstream output(header.cix_filename, ofstream::binary);
         header.cix_command = CIX_ACK;
